...
std::tuple<Expression,std::vector<float>> GeneticProgrammingIslands::fit(std::shared_ptr<Dataset> dataset, int ngenerations, int nsupergenerations, 
    ...
    #pragma omp parallel num_threads(nislands)
    {
        ...
        for (int supergeneration = 0; supergeneration < nsupergenerations; ++supergeneration) {
            // Run island
            local_histories[threadIdx] = islands[threadIdx]->fit(dataset, ngenerations, nepochs, learning_rate);
