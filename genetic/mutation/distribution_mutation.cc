...
namespace qsr {
    std::shared_ptr<BaseMutator> DistributionMutation::get_mutator(int nvars, int nweights, int max_depth) {
        ...
        for (const auto &mutation : mutations) {
            mutators.push_back(mutation->get_mutator(nvars, nweights, max_depth));
        }
